...
def main():
    parser = argparse.ArgumentParser(description='Train KAN model on ImageNet')
    ...
    # Layer configuration
    parser.add_argument('--layers', type=str, default="32,7,2,3;64,3,2,1;128,3,2,1", 
    ...
    model = KANConvNet(
        ...
        regularize_activation=args.regularize_activation,
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='val_acc',
        dirpath='checkpoints/kan_imagenet',
        filename='kan_imagenet-{epoch:02d}-{val_acc:.3f}',
        save_top_k=3,
        mode='max',
    )
    ...
    logger = TensorBoardLogger(
        'logs', 
        name=f'kan_imagenet_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    )
    ...
    # Save final model
    trainer.save_checkpoint(f"kan_imagenet_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ckpt")
