...
def main():
    parser = argparse.ArgumentParser(description='Train KAN model on small ImageNet subset')
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='val_acc',
        dirpath='checkpoints/kan_small',
        filename='kan_small-{epoch:02d}-{val_acc:.3f}',
        save_top_k=3,
        mode='max',
    )
    ...
    logger = TensorBoardLogger(
        'logs', 
        name=f'kan_small_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    )
    ...
    # Save final model
    trainer.save_checkpoint(f"kan_small_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ckpt")
